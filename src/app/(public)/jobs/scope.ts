...
export type CareersTenant = { tenantId: string; companyName: string; intro: string | null }
...
export async function getCareersTenant(): Promise<CareersTenant | null> {
  ...
  const { data } = await admin
    ...
    .select('tenant_id, legal_name, careers_intro')
    .eq('jobs_enabled', true)
    .limit(1)
    .maybeSingle()
  if (!data) return null
  return { tenantId: data.tenant_id, companyName: data.legal_name || 'Globaal Elevate Production s.r.o.', intro: data.careers_intro ?? null }
}
