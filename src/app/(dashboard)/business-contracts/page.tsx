...
export default async function BusinessContractsPage() {
  ...
  const [{ data: contracts }, { data: suppliers }, { data: clients }, { data: events }] = await Promise.all([
  ...
  const partyName = (c: any) =>
    ...
    c.counterparty || null
  const full = (contracts ?? []).map((c: any) => ({ ...c, party_name: partyName(c) }))

  return (
    <div className="space-y-6">
      <PageHeader title="Obchodní smlouvy" description="Smlouvy s umělci, pronájmy a dodavateli — platnost, expirace a e-akceptace." />
      <BusinessContractsClient contracts={full} suppliers={suppliers ?? []} clients={clients ?? []} events={events ?? []} />
    </div>
  )
}
