...
export default async function PersonalOverviewPage() {
  ...
  const [notesCount, openTasks, overdue, upcomingCount, events, tasks, pinned] = await Promise.all([
    ...
    supabase.from('personal_events').select('id, title, start_time, all_day').eq('user_id', uid).gte('start_time', todayStr).order('start_time', { ascending: true }).limit(5),
  ...
  const fmtDateTime = (s: string, allDay?: boolean) =>
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard title="Nadcházející události" value={String(upcomingCount.count || 0)} hint="V osobním kalendáři" icon={<CalendarDays className="size-4" />} />
      </div>
            ...
            <ul className="space-y-2">
              {(events.data ?? []).map((e: any) => (
                <li key={e.id} className="flex items-center justify-between gap-2 text-sm">
                  ...
                  <span className="shrink-0 tabular-nums text-xs text-muted-foreground">{fmtDateTime(e.start_time, e.all_day)}</span>
                </li>
              ))}
            </ul>
          )}
