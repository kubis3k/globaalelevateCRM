...
export default async function HrOverviewPage() {
  ...
  const [emp, pendingLeave, openJobs, outToday, expiringRes, dppRes] = await Promise.all([
  ...
  const ids = Array.from(new Set([...expiring.map((c: any) => c.user_id), ...dppWarn.map((d) => d.user_id)]))
  ...
  const hasIssues = expiring.length > 0 || dppWarn.length > 0
